...
function render(){
  ...
  if(summary)summary.textContent=`Chỉ tin khách gửi · ${rows.length} đoạn · ${selectedIds.size} đang dùng`;
  ...
  if(!rows.length){scroll.innerHTML='<div class="order-source-empty">Không có tin khách phù hợp trong khoảng này. Có thể bật “Hiện tất cả tin khách”.</div>';return;}
  scroll.innerHTML=rows.map(row=>{
    const id=String(row.messageId||'');
    const state=String(row.state||'pending');
    const selected=selectedIds.has(id);
    const immutable=state==='imported';
    return`<article class="order-source-message" data-source-message="${escapeHtml(id)}" data-selected="${selected}" data-state="${escapeHtml(state)}">
      <div class="order-source-message-top">
        <input class="order-source-select" type="checkbox" data-source-select="${escapeHtml(id)}" ${selected?'checked':''} ${immutable?'disabled':''} aria-label="Chọn đoạn báo hàng">
        <time class="order-source-time">${escapeHtml(timeText(row.createdAt))}</time>
        <span class="order-source-state" data-state="${escapeHtml(state)}">${escapeHtml(stateLabel(state))}</span>
      </div>
      <div class="order-source-text">${escapeHtml(row.text||'')}</div>
      <div class="order-source-actions">
        <button type="button" data-source-action="quick" data-source-id="${escapeHtml(id)}">Tách nhanh</button>
        <button type="button" data-source-action="ai" data-source-id="${escapeHtml(id)}">AI</button>
        ${state!=='imported'?`<button type="button" data-source-action="ignore" data-source-id="${escapeHtml(id)}">Bỏ qua</button>`:''}
      </div>
      ${renderResult(row)}
    </article>`;
  }).join('');
...
async function refresh(){
  ...
  const contact=currentContact();
  if(!contact){rows=[];selectedIds.clear();lastRange={from:'',to:''};render();return false;}
...
async function setRowState(messageId,state){
  const contact=currentContact();
  if(!contact?.id)return false;
  await invoke('set_state',{contactId:contact.id,messageId,state});
  const row=rows.find(item=>String(item.messageId)===String(messageId));
  if(row)row.state=state;
  render();
  return true;
}
async function toggleSelected(messageId,checked){
  const id=String(messageId||'');
  if(!id)return;
  if(checked){selectedIds.add(id);await setRowState(id,'working');}
  else{selectedIds.delete(id);const row=rows.find(item=>String(item.messageId)===id);if(row?.state==='working')await setRowState(id,'pending');}
  render();
  dispatchContext();
}
async function splitRow(messageId,mode){
  const row=rows.find(item=>String(item.messageId)===String(messageId));
  const contact=currentContact();
  if(!row||!contact?.id)return false;
  row._splitResult={items:[],unresolved:[],error:mode==='ai'?'AI đang tách…':'Đang tách nhanh…'};
  render();
  try{
...
async function ignoreRow(messageId){
  const id=String(messageId||'');
  selectedIds.delete(id);
  await setRowState(id,'ignored');
  dispatchContext();
}
function onClick(event){
  ...
  const id=String(button.dataset.sourceId||'');
  if(action==='quick'||action==='ai')void splitRow(id,action);
  else if(action==='ignore')void ignoreRow(id);
}
function onChange(event){
  const target=event.target;
  if(target?.matches?.('[data-source-select]')){void toggleSelected(target.dataset.sourceSelect,target.checked);return;}
