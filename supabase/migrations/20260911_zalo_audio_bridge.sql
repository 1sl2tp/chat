-- V21.73.2 Zalo audio bridge
-- Additive extension only: keep v21_messages + v21_media_assets canonical.
...
begin
  ...
  if char_length(v_body)>8000 or v_kind not in ('image','audio','file') or v_mime='' then return null; end if;
  ...
  if v_kind='audio' and v_mime not like 'audio/%' then return null; end if;
...
begin
  if new.deleted_at is not null or new.message_id is null or new.kind not in ('image','audio','file') then return new; end if;
