  ...
  async runProfile(profile) {
    ...
    const verdict = profileError
      ...
      : classifyMatrixResult({
          ...
          inboundEnergy: stats.inboundEnergy,
        })

    this.log(`result meter=${meterState} track=${track.readyState}/${track.enabled ? 'on' : 'off'} localEnergy=${this.localEnergyPeak} outbound=${stats.outboundBytes} inbound=${stats.inboundBytes} inboundEnergy=${stats.inboundEnergy} verdict=${verdict}`)
