...
Phase 1 is text-only. Images, files, audio, reactions, reply metadata, calls, read receipts, and typing indicators are out of scope for this phase.
...
Choosing `Liên kết Zalo` opens a small picker showing Zalo identities discovered by the bridge. Each row uses the Zalo display name and avatar only to help Admin identify the person.
...
Changing a link replaces the active Zalo identity and resets the activation time.

Unlinking immediately stops future Zalo ingress/egress for that User. Existing canonical Chat messages remain unchanged.
...
The bridge upserts this table as it learns identities. This table is not a Chat contact table.
...
3. Resolve an enabled `zalo_user_links` row for `zalo_id`.
...
Outbound applies only to new canonical text messages authored by Admin.

For each new Chat message:

1. Existing Chat send path creates the canonical message first.
2. Server-side bridge routing checks whether the conversation peer is a User with an enabled Zalo link.
...
7. Bridge sends the text to the linked `zalo_id`.
...
Keep bridge source isolated from browser runtime under a dedicated server directory in the Chat repository, for example:
...
12. Unlink immediately stops future bridge routing.
...
- unlinking stops future cross-delivery;
