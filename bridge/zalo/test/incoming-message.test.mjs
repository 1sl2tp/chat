...
test('ignores self, group and non-text messages',()=>{
  assert.equal(normalizeIncomingMessage({type:ThreadType.User,isSelf:true,threadId:'z1',data:{msgId:'m1',ts:'1',content:'x'}}),null);
  assert.equal(normalizeIncomingMessage({type:ThreadType.Group,isSelf:false,threadId:'g1',data:{msgId:'m2',ts:'1',content:'x'}}),null);
  assert.equal(normalizeIncomingMessage({type:ThreadType.User,isSelf:false,threadId:'z1',data:{msgId:'m3',ts:'1',content:{type:'photo'}}}),null);
});
...
test('binds message listener and forwards only normalized events',async()=>{
  ...
  await messageHandler({type:ThreadType.User,isSelf:false,threadId:'z2',data:{msgId:'m9',ts:'1789105000000',content:'hello'}});
