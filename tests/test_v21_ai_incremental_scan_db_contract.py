...
TIMEOUT_REPAIR=ROOT/'supabase/migrations/20260915_chat_ai_scan_cron_timeout.sql'
EDGE=ROOT/'supabase/functions/v21-order-scan/index.ts'

sql=MIGRATION.read_text(encoding='utf-8')
repair=TIMEOUT_REPAIR.read_text(encoding='utf-8')
...
# Cold starts + an actual AI batch can exceed pg_net's 1s timeout. The repair
# keeps the request alive long enough for the Edge scan to finish and respond.
assert 'create or replace function public.chat_ai_scan_enqueue_http()' in repair.lower()
assert 'timeout_milliseconds := 120000' in repair
