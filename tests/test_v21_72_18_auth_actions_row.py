...
assert 'Bạn chưa có tài khoản? Hãy' in source
...
assert "secondary.textContent=authMode==='register'?'đăng nhập':'đăng ký'" in shell
assert "switchPrefix.textContent=authMode==='register'?'Bạn đã có tài khoản? Hãy':'Bạn chưa có tài khoản? Hãy'" in shell
assert '>Tiếp tục</span>' in source
assert '>đăng ký</span>' in source
assert 'setPasswordVisibility' in shell
assert 'shouldAutoFocusShellForm' in shell

assert "RELEASE_VERSION='V21.72.39'" in sw
assert "MODULE_CONTRACT_VERSION='pwa-sw-v1'" in sw
assert "CACHE_NAME='taphoa-chat-shell-'+RELEASE_VERSION" in sw

print('V21.72.39 auth single-action form / PWA SW release contract PASS')
