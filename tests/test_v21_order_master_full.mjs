...
import * as core from '../supabase/functions/v21-order-scribe/scribe-core.mjs';

assert.equal(typeof core.parseMasterOrderPayload,'function','master JSON parser must live in shared core');

if(typeof core.parseMasterOrderPayload==='function'){
  ...
  const noAction=core.parseMasterOrderPayload({
    intent:'NO_ACTION',
    requires_human_action:false,
    parsed_items:[],
  });
  ...
  const inquiry=core.parseMasterOrderPayload({
    intent:'INQUIRY',
  ...
  assert.throws(()=>core.parseMasterOrderPayload({
    intent:'ORDER',
    requires_human_action:false,
    parsed_items:[],
  }),/ai_items_missing/,'ORDER with no extracted items remains an extraction failure');
}
