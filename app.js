...
const InteractionMode=window.V21InteractionMode;
const InteractionController=window.V21InteractionController;
if(!InteractionMode||!InteractionController)throw new Error('interaction_controller_missing');
...
const AppBootController={
  ...
  fail(error){
    ...
    runtimeError.textContent='V21.72.39 runtime: '+message;
    runtimeError.classList.remove('hidden');
    console.error('[ChatScreenModule V21.72.39]',error);
  },
  ready(){
    this.phase='READY';
...
/* =========================================================
   V21.72.39 VIEWPORT POLICY + CANONICAL CONVERSATION/COMPOSER SCOPE
...
function ensureImageViewer(){
  ...
  /* Viewer stage shortcuts.
     Any primary pointer may close the viewer by tapping/clicking the empty
     black stage. Touch/pen additionally keep double-tap zoom and swipe-down
     close. Starting on the image or a control never counts as a stage close. */
  main.addEventListener('pointerdown',event=>{
  ...
  const finishImageViewerPointerShortcut=event=>{
    const start=imageViewerTouchStarts.get(event.pointerId);
    if(!start)return;
    ...
    if(touchLike&&!start.multi&&imageViewerZoom===1&&dy>72&&Math.abs(dy)>Math.abs(dx)*1.15){
      imageViewerLastTouchTapAt=0;
    ...
    if(!start.multi&&!moved&&start.onStage){
      ...
      closeImageViewer({restoreFocus:!touchLike});
      return;
    }

    if(!touchLike||start.multi||moved||!start.onImage)return;
  ...
  main.addEventListener('pointerup',finishImageViewerPointerShortcut);
...
function createActionGroup(message){
  ...
  if(window.V21MessageForward?.canForward?.(message)){
    actions.push(makeActionButton({
      label:'Chuyển tiếp',
      icon:'forward',
      onClick:event=>{
        event.stopPropagation();
        closeAllTurnActions();
        window.V21MessageForward?.open?.(resolveMessage());
      }
    }));
  }
...
/* =========================================================
   SMART SCROLL
   ========================================================= */
function suppressScrollFromEndControlForTail(){
  scrollRoot.removeAttribute('data-scroll-from-end');
  stageLayout.removeAttribute('data-scroll-from-end');
  if(threadScrollControl){
    threadScrollControl.disabled=true;
...
function updateScrollFromEndControl(){
  ...
  if(pendingTailReason){
    suppressScrollFromEndControlForTail();
    return false;
  }
  if(threadScrollControl){
    threadScrollControl.disabled=false;
